  ...
  <div class="w-full">
    ...
    <div class="flex items-center w-full flex-wrap">
      ...
      <div class="flex items-center lt-sm:(w-full justify-between py-3)">
        <!-- 循环控制按钮 -->
        <span @click="handleLoopChange" class="hidden over lt-sm:(block text-xl flex items-center )"
          v-if="hasName('mode')">
          <!-- 普通列表 -->
        ...
        <div class="flex items-center">
          <i class="i-iconoir:skip-prev lt-sm:(text-3xl)" @click="handleChangeAudio('prev')" v-if="hasName('prev')"></i>
          <i class="i-iconoir:play mx-2 lt-sm:(text-5xl mx-9)" @click="togglePlay" v-if="!state.isPlay"></i>
          <i class="i-iconoir:pause mx-2 lt-sm:(text-5xl mx-9)" @click="togglePlay" v-else></i>
          <i class="i-iconoir:skip-next lt-sm:(text-3xl)" @click="handleChangeAudio('next')" v-if="hasName('next')"></i>
        </div>
        <!-- 速率控制按钮 -->
        <span class="cursor-pointer hidden min-w-10 lt-sm:(block text-xl) text-right select-none"
          @click="handleRateChange" v-if="hasName('rate')">{{ rateList[rateCurrent] }}x</span>
      </div>
      <!-- 进度条部分 -->
      <div class="flex items-center flex-1 mx-4 flex-wrap lt-sm:mx-0">
        ...
        <i class="i-iconoir:backward-15-seconds lt-sm:(text-2xl)" @click="seekPlayback(PlayAction.Rewind)"
          v-if="hasName('rewind')"></i>
        <!-- 进度条 -->
        ...
        <i class="i-iconoir:forward-15-seconds lt-sm:(text-2xl)" @click="seekPlayback(PlayAction.Forward)"
          v-if="hasName('forward')"></i>
      </div>
      <div class="flex items-center mx-2 lt-sm:(w-full mx-0 my-2 justify-between) lt-sm:(hidden)"
        ...
        <i class="flex items-center group" v-if="hasName('volume')">
          <!-- 静音&音量调节按钮 -->
          <ProgressBar v-model="state.volume"
            class="transition-width invisible w-0 mx-0! group-hover:(visible w-[80px] ml-2! mr-4!)"></ProgressBar>
        ...
        <span class="mr-3 cursor-pointer min-w-8 text-center select-none" @click="handleRateChange"
          v-if="hasName('rate')">{{ rateList[rateCurrent] }}x</span>
        <!-- 循环控制按钮 -->
